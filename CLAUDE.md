...
**⚠️ 改修するたびに必ずバージョンを上げること。コミット前に必ず確認すること。**
...
1. [`src/App.tsx`](src/App.tsx) の `APP_VERSION` 定数
2. [`package.json`](package.json) の `version` フィールド

> Service Worker のキャッシュ無効化は `vite-plugin-pwa` がビルドごとに自動で行うため、手動更新は不要。

**PR を作成する前に必ずバージョンが上がっていることを確認すること。バージョン更新を忘れた場合は別途 patch バージョンアップのコミットを追加すること。**
